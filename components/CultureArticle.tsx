...
function CultureArticle({ header, subtext }: CultureArticleProps) {
  ...
  return (
    <article className="border-t border-solid border-[#cecece] last:border-b [counter-increment:inline-accordion-counter]">
      <h1 className="m-0" onClick={() => setState(p => !p)} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
        <button className="cursor-pointer relative grid grid-cols-[2rem_1fr_4.4rem] items-center p-[1rem_0] gap-[2rem] w-full text-[#0c0c0c] text-left bg-transparent border-none min-h-[8.5rem] font-neue-haas text-[1.8rem] font-normal leading-[1.5] tracking-[-0.01em] md:text-[2.2rem] md:leading-[1.1] md:min-h-[10.5rem] group" aria-expanded={state} aria-controls="inline-accordion-move-fast-be-technically-fearless">
          ...
          <span className="relative">
            <span className="transition-[background-size] duration-300 ease-in-out group-hover:[background-size:100%_0.1rem,0_0.1rem] md:group-hover:[background-size:0_0.1rem,100%_0.1rem]" style={{ background: 'linear-gradient(90deg,hsla(0,0%,5%,0),hsla(0,0%,5%,0)),linear-gradient(90deg,#0c0c0c,#0c0c0c)', backgroundSize: '100% 0.1rem,0 0.1rem', backgroundPosition: '100% 100%,0 100%', backgroundRepeat: 'no-repeat' }}>{header}</span>
          </span>
      ...
      <div id="inline-accordion-move-fast-be-technically-fearless" className="transition-[height] duration-300 ease-in-out overflow-hidden" style={{ height: state ? "112px" : "0px" }}>
