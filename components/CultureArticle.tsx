  ...
  return (
    <article className="border-t border-solid border-[#cecece] last:border-b [counter-increment:inline-accordion-counter]">
      <h1 className="m-0" onClick={() => setState(p => !p)} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
        <button className="cursor-pointer relative grid grid-cols-[2rem_1fr_4.4rem] items-center p-[1rem_0] gap-8 w-full text-brand-dark text-left bg-transparent border-none min-h-[8.5rem] font-neue-haas text-[1.8rem] font-normal leading-normal tracking-[-0.01em] md:text-[2.2rem] md:leading-[1.1] md:min-h-[10.5rem] group" aria-expanded={state} aria-controls="inline-accordion-move-fast-be-technically-fearless">
