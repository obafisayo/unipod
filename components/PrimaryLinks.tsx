...
function PrimaryLinks({ title, text, icon, paddingTop, borderBottom, borderTop, slide, fsz, mfsz, fw, height, ls, link, unslide, hoverIt }: PrimaryLinksProps) {
  ...
  const thereisIcon = <div className="flex justify-between w-[95dvw] text-brand-dark">{thereisSliderSlide} <Svg hov={hover} plus /> </div>
  ...
  const ContentWrapper = () => (
    <span className={`inline-block leading-[0.8] text-brand-dark transition-[background-size] duration-300 ease-in-out bg-[linear-gradient(90deg,hsla(0,0%,5%,0),hsla(0,0%,5%,0)),linear-gradient(90deg,#0c0c0c,#0c0c0c)] [background-position:100%_100%,0_100%] bg-no-repeat ${hover || hoverIt ? 'bg-[length:0_0.3rem,100%_0.3rem]' : 'bg-[length:100%_0.3rem,0_0.3rem]'}`} 
    >
      {PrimaryLinksContent()}
    </span>
  );
  ...
  return (
    <div 
      className={`footer__primary-link flex items-center justify-start text-brand-dark no-underline cursor-pointer pb-[1.5rem] pr-[1.7rem] md:pb-10 font-machina text-[3.4rem] md:text-[5.2rem] font-normal leading-[1.11] uppercase tracking-normal border-t border-solid border-[hsla(0,0%,5%,.2)] first:border-t-0 before:content-[counter(number__primary-links-counter,decimal-leading-zero)] before:block before:mr-[3rem] before:font-machina before:text-[1.8rem] md:before:text-[2.4rem] md:before:relative md:before:top-[-0.2rem] before:font-normal before:leading-[1.11] before:uppercase before:tracking-[-0.01em] before:text-brand-dark font-features-['ss12'_on] ${borderStyles}`}
