    ...
    <div className="team-modal__main-wrapper p-[2rem] h-full overflow-y-auto md:p-[6rem_2rem]">
      ...
      <div className="team-modal__main flex flex-col gap-[3.2rem] md:gap-[4.2rem]">
        <div className="team-modal__heading-title-container flex flex-col gap-[1.5rem]">
          ...
          <div className="team-modal__title font-machina text-[3.4rem] font-normal leading-[1.11] tracking-normal order-1 m-0 w-full max-w-[calc(100%-6rem)] uppercase md:text-[5.2rem] md:order-2 md:max-w-none font-features-['ss12'_on]" >{name}</div>
        </div>
        <div className="team-modal__image-wrapper aspect-square w-full h-auto overflow-hidden rounded-[0.4rem]">
          {src ? (
            <Image 
              ...
              height={742} 
              decoding="async" 
              className="team-modal__image object-center object-cover block w-full h-auto" 
              loading="lazy" 
              style={{ color: "transparent" }} 
            />
          ) : null}
        </div>
        <div className="team-modal__bio m-0 whitespace-pre-line font-neue-haas font-medium text-[#0c0c0c]">
          {para1} <br />
          <br />
          {para2 && <>{para2} <br /><br /></>}
          {para3 && <>{para3} <br /><br /></>}
